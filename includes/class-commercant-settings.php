...
class commercant_Settings {

	/**
	 * The single instance of commercant_Settings.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	public function add_menu_item () {
		$page = add_options_page( __( 'Commercant options', 'commercant' ) , __( 'Commercant options', 'commercant' ) , 'manage_options' , 'commercant_settings' ,  array( $this, 'settings_page' ) );
	...
	public function add_settings_link ( $links ) {
		$settings_link = '<a href="options-general.php?page=commercant_settings">' . __( 'Settings', 'commercant' ) . '</a>';
	...
	private function settings_fields () {
		...
		$settings['standard'] = array(
			'title'					=> __( 'Standard', 'commercant' ),
			'description'			=> __( '', 'commercant' ),
			'fields'				=> array(
				array(
		...
		$settings = apply_filters( 'commercant_settings_fields', $settings );
	...
	public function register_settings () {
		if( is_array( $this->settings ) ) {
			foreach( $this->settings as $section => $data ) {
				...
				// Add section to page
				add_settings_section( $section, $data['title'], array( $this, 'settings_section' ), 'commercant_settings' );
				...
				foreach( $data['fields'] as $field ) {
					...
					register_setting( 'commercant_settings', $option_name, $validation );
	...
	public function display_field ( $args ) {
		...
		switch( $field['type'] ) {
			...
			case 'image':
				...
				$html .= '<input id="' . $option_name . '_button" type="button" data-uploader_title="' . __( 'Upload an image' , 'commercant' ) . '" data-uploader_button_text="' . __( 'Use image' , 'commercant' ) . '" class="image_upload_button button" value="'. __( 'Upload new image' , 'commercant' ) . '" />' . "\n";
				$html .= '<input id="' . $option_name . '_delete" type="button" class="image_delete_button button" value="'. __( 'Remove image' , 'commercant' ) . '" />' . "\n";
	...
	public function settings_page () {
		...
		// Build page HTML
		$html = '<div class="wrap" id="commercant_settings">' . "\n";
			$html .= '<h2>' . __( 'Commer&ccedil;ant options' , 'commercant' ) . '</h2>' . "\n";
			$html .= '<form method="post" action="options.php" enctype="multipart/form-data">' . "\n";

				// Setup navigation
				$html .= '<ul id="settings-sections" class="subsubsub hide-if-no-js">' . "\n";
					$html .= '<li><a class="tab all current" href="#all">' . __( 'All' , 'commercant' ) . '</a></li>' . "\n";
				...
				// Get settings fields
				ob_start();
				settings_fields( 'commercant_settings' );
				do_settings_sections( 'commercant_settings' );
				$html .= ob_get_clean();

				$html .= '<p class="submit">' . "\n";
					$html .= '<input name="Submit" type="submit" class="button-primary" value="' . esc_attr( __( 'Save Settings' , 'commercant' ) ) . '" />' . "\n";
				$html .= '</p>' . "\n";
			$html .= '</form>' . "\n";
		$html .= '</div>' . "\n";
	...
	/**
	 * Main commercant_Settings Instance
	 *
	 * Ensures only one instance of commercant_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @see commercant()
	 * @return Main commercant_Settings instance
	 */
